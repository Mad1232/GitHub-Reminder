...
//            GetJSONTest(dataText);
    ...
    private void GetJSONTest(TextView dataText) throws IOException, NullPointerException {
        JsonRequest<String> StringReq = new JsonRequest<String>(
                Request.Method.GET,
                URL_JSON_ARRAY,
                null, // Pass null as the request body since it's a GET request
                response -> {
                    dataText.setText("We got a response!");
        ...
        VolleySingleton.getInstance(getContext().getApplicationContext()).addToRequestQueue(StringReq);
    }
}